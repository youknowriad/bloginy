...
{
    if ($errfile == __FILE__ || file_exists(dirname(__FILE__).'/sub/'.basename($errfile))) {
...
class Arabic
{
    ...
    public function load($library)
    {
        ...
        if (!$use_autoload) {
            ...
            include_once $this->_path.'/sub/'.$this->myClass.'.class.php';
        }
    ...
    public function coreConvert($str, $inputCharset, $outputCharset)
    {
        if ($inputCharset != $outputCharset) {
            ...
            if($conv_str == '' && $str != '') {
                include_once($this->_path.'/sub/ArCharsetC.class.php');
